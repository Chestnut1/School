<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password) && !empty($birth) && !empty($uname)){
        //controllo se la mail esiste già
        try {			
            //connessione al DB	
            $con = new PDO("mysql:host=localhost;dbname=chat", "root", "");
            $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $istruzione="SELECT email FROM utenti WHERE username = '" . $uname . "'";
            ...
            if($totale==0){ //l'email non è presente nel DB
                ...
                if(isset($_FILES['imgprofile'])){   //controllo se l'utente ha caricato un file
                    ...
                    if(in_array($img_ext, $extensions) === true){
                        ...
                        $istruzione="INSERT INTO utenti(username, email, nome, cognome, nascita, immagine, stato, password) values ('" . $uname . "', '" . $email . "', '" . $fname . "', '" . $lname . "', '" . $birth . "', '" . $new_image_name . "', '" . $status . "', '" . $password . "')";
                        $stmt = $con->prepare($istruzione);
                        $stmt->execute();  //eseguo la query
                        ...
                        echo "Profile img must be one of this: .png, .jpg. jpeg!" ;
                    }
            ...
            }else{  //l'username è già registrato nel DB
