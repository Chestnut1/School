    ...
    <?php
        ...
        if(isset($_SESSION['idUtente']) && 
			...
			try {				
				...
				$istruzione1 = 'SELECT count(abbinamenti.id) AS nAbb FROM abbinamenti, utenti AS utenti1, utenti AS utenti2 WHERE (abbinamenti.idUtente1 = utenti1.id) AND (abbinamenti.idUtente2 = utenti2.id)';
				...
				if ($row = $result->fetch(PDO::FETCH_OBJ))
						$nAbb=$row->nAbb;
				echo "Su un numero di abbinamenti pari a: " . $nAbb;
				if ($nAbb > 0){ 				
		...
		echo("<button type='submit' class='btn btn-success' formaction='esportaAbbinamentiAdmin.php'>Esporta CSV abbinamenti</button>");
        echo("</form>");
    ?>
